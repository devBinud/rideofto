            ...
            @if (count($pendingBookingReq) > 0)
                <div class="table mt-3">
                    <table class="table table-bordered text-center table-responsive">
                        ...
                        <tbody>
                            @foreach ($pendingBookingReq as $item)
                                <tr>
                                    <td class="px-3">{{ ++$sn }}</td>
                                    ...
                                    <td> <button type="button" data-bs-toggle="modal" data-bs-target="#staticBackdrop" data-bookingid="{{$item->id}}" class="btn btn-primary btn-sm px-2 viewAll">
                                            <i class="fa fa-eye"></i>
                                        </button>
                                    </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#paginationBox").pxpaginate({
                currentpage: {{ $pendingBookingReq->currentPage() }},
                totalPageCount: {{ ceil($pendingBookingReq->total() / $pendingBookingReq->perPage()) }},
                maxBtnCount: 10,
                align: "center",
                ...
                callback: function(pagenumber) {
                    var url = "{!! url('vendor/booking?action=booking-request&page=') !!}" + pagenumber;
            ...
            $(document).on('click','.viewAll',function(e){
                ...
                $('#appendHere').html('<h4 class="text-center p-3">Please wait...</h4>');
                let url = "{{ url('vendor/booking?action=get-user-details') }}";
                $.get(url + "&bookingId=" + bookingId + "&type=" + "viewBooking" + "&status=" + "pending", function(data) {
                    $('#appendHere').html(data.html);
                });
            });
